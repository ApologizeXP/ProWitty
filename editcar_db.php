...
if (isset($_POST['edica'])) {
    ...
    if (count($errors) == 0) {
        if ($file_size >> 0) {
            $sql = " UPDATE cars SET imagecar='$new_file_name',carname='$carname',detials='$detials',numcar='$numcar',status='$status' WHERE idcar = '$id' ";
            mysqli_query($conn, $sql);
        } else {
            $sql = "UPDATE cars SET carname='$carname',detials='$detials',numcar='$numcar',status='$status' WHERE idcar = '$id'";
            mysqli_query($conn, $sql);
        }
        header('location: managecar.php');
    } else {

        header('location: editcar.php?id='. $id.' ');
    }
}
